...
export function FloatingCard({
  ...
  const inner = (
    <>
        ...
        <div className="relative -mx-7 -mt-7 mb-5 overflow-hidden rounded-t-editorial">
          <div className="aspect-[16/10] overflow-hidden">
            <img
              ...
              loading="lazy"
              decoding="async"
              className="h-full w-full object-cover transition-transform duration-700 ease-divine group-hover:scale-[1.04]"
            />
          </div>
        </div>
      )}
      ...
      <h3 className="mt-2 font-display text-2xl text-balance text-temple-800">{title}</h3>
      {body && <p className="mt-3 text-sm leading-relaxed text-temple-700/85">{body}</p>}
      {(to || href) && (
        <>
